...
DEFAULT_CONFIG = {
    ...
    # which drives openpiv-python's own multi-pass, window-deformation
    # pipeline (openpiv.windef, via an openpiv.settings.PIVSettings
    # object) -- the same coarse-to-fine multi-pass + validation +
    # outlier-replacement + optional-smoothing feature set as piv_gpu.
    # Keys here are PIVSettings field names -- see
    # openpiv.settings.PIVSettings for the full list (min_max_u_disp,
    # std_threshold, median_threshold, median_size, etc. are also
    # available and fall back to PIVSettings' own defaults if omitted).
    # Unknown keys are warned about, not silently dropped.
    #
